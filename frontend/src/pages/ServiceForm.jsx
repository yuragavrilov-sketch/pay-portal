...
export default function ServiceForm() {
  ...
  useEffect(() => {
    if (isEdit) api.svcGet(id).then(setForm).catch(e => setError(e.message));
  }, [id]);
  ...
  const submit = async (e) => {
    ...
    setError('');
    try {
      if (isEdit) await api.svcUpdate(id, form);
      else await api.svcCreate(form);
      navigate('/services');
  ...
  return (
    <div style={{ maxWidth: 600 }}>
      <h4 className="mb-3"><i className="bi bi-diagram-3 me-2"></i>{isEdit ? 'Редактировать' : 'Создать'} сервис</h4>
      ...
      <form onSubmit={submit}>
        ...
        <button className="btn btn-secondary" type="button" onClick={() => navigate('/services')}>Отмена</button>
      </form>
    </div>
  );
}
